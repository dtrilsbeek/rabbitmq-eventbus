...
namespace RabbitMQEventbus.Extension.DependencyInjection
{
	public static class ServiceCollectionExtensions
	{
		...
		public static IServiceCollection AddRabbitMQEventbus(this IServiceCollection services, IWebHostEnvironment env)
		{
			...
			string path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) ?? string.Empty,
				configFileName);
			var file = File.ReadAllText(path);
			var config =
				JsonConvert.DeserializeObject<RabbitMQEventbusConfiguration.RabbitMQEventbusConfiguration>(file);
			...
			services.AddSingleton<IPersistentConnection>(s => new PersistentConnection(
					s.GetRequiredService<IConnectionFactory>(),
					s.GetRequiredService<ILogger<PersistentConnection>>()
				)
			);

			services.AddSingleton<IRabbitMQEventbus>(s => new Eventbus.RabbitMQEventbus(
				config,
